...
function display_comments_modal(data){
    let list: HTMLElement = document.getElementById("comment_list");
...
$('#newArticle').click(function(){
    ...
    $("#save_new_article").click(function(){
        let article_cat: string =  $("#newarticle_category").dropdown('get value');
        let article_text: string = $("#article_text").val().toString();
        let headline: string = $("#new_article_headline").val().toString();
...
$("#newComments").click(function(){
    ...
    $("#save_new_comment").click(function(){
        let temp: string = $("#new_comment_text").val().toString();
        if (temp.length != 0){
...
function post_new_comment(comment_text: string){
    $.ajax({
        url: '/post_comment',
        type: 'POST',
        contentType: 'application/json',
...
function post_new_article(article_cat: string, article_text: string, headline: string){
    let date_created: string = new Date().toISOString().slice(0, 19).replace('T', ' ');
    $.ajax({
        url: '/post_article',
        type: 'POST',
        contentType: 'application/json',
